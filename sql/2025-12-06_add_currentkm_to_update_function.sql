-- =============================================================================
-- FIX: Thêm p_vehicle_id và p_current_km vào work_order_update_atomic
-- =============================================================================
-- Date: 2025-12-06
-- Purpose: Cho phép lưu currentKm khi cập nhật phiếu sửa chữa
-- =============================================================================
...
CREATE OR REPLACE FUNCTION public.work_order_update_atomic(
  ...
  p_user_id TEXT,
  p_vehicle_id TEXT DEFAULT NULL,      -- 🔹 NEW: vehicleId support
  p_current_km INTEGER DEFAULT NULL    -- 🔹 NEW: currentKm support
) RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  -- ==========================================================================
  -- STEP 4: Update work order (🔹 INCLUDE vehicleId and currentKm)
  -- ==========================================================================
  UPDATE work_orders
  SET
    customerName = p_customer_name,
    customerPhone = p_customer_phone,
    vehicleId = COALESCE(p_vehicle_id, vehicleId),       -- 🔹 NEW
    vehicleModel = p_vehicle_model,
    licensePlate = p_license_plate,
    currentKm = COALESCE(p_current_km, currentKm),       -- 🔹 NEW
