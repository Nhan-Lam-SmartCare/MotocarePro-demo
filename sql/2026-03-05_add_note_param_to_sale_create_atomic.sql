-- Add note support to sale_create_atomic (backward-compatible schema upgrade)
-- Date: 2026-03-05
-- Purpose: Persist sale note directly in atomic RPC for quick service and normal sale flows.
...
BEGIN
  ...
  INSERT INTO sales(id, date, items, subtotal, discount, total, customer, paymentmethod, userid, username, branchid, note)
  VALUES (p_sale_id, NOW(), p_items, v_subtotal, p_discount, v_total, p_customer, p_payment_method, p_user_id, p_user_name, p_branch_id, NULLIF(TRIM(COALESCE(p_note, '')), ''))
...
COMMENT ON FUNCTION public.sale_create_atomic IS 'Tạo hóa đơn và cập nhật tồn kho + giao dịch tiền mặt trong 1 transaction (atomic). Hỗ trợ quick_service, check available stock (stock - reservedstock), và lưu note qua p_note.';
