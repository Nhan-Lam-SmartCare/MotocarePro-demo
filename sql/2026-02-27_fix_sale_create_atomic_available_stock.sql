-- Fix Quick Service Support and Available Stock Check in sale_create_atomic
-- Date: 2026-02-27
-- Issue: The sale_create_atomic function was not deducting reserved stock when validating available stock.
-- This allowed direct sales to consume stock that was already reserved for work orders.
...
BEGIN
  ...
  FOR v_index IN 0..(v_items_count - 1) LOOP
    ...
    v_available_stock := GREATEST(0, v_current_stock - v_current_reserved);
...
COMMENT ON FUNCTION public.sale_create_atomic IS 'Tạo hóa đơn và cập nhật tồn kho + giao dịch tiền mặt trong 1 transaction (atomic). Hỗ trợ quick_service không cần trừ kho. Đã sửa lỗi check available stock cho đúng (tính cả reservedstock).';
