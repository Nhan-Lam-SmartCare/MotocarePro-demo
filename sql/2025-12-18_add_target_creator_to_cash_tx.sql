-- Add target (customer) and creator (staff) to cash transactions
-- Date: 2025-12-18

-- 1. Add columns to cash_transactions table if they don't exist
ALTER TABLE cash_transactions ADD COLUMN IF NOT EXISTS target_name TEXT;
ALTER TABLE cash_transactions ADD COLUMN IF NOT EXISTS created_by UUID;

-- 2. Update sale_create_atomic to populate these fields
...
BEGIN
  ...
  v_customer_name := COALESCE(p_customer->>'name', 'Khách lẻ');
  ...
  -- Cash transaction with target_name and created_by
  INSERT INTO cash_transactions(id, type, category, amount, date, description, branchid, paymentsource, reference, target_name, created_by)
  VALUES (
    v_cash_tx_id, 
    'income', 
    'sale_income', 
    ...
    v_customer_name, -- Populate target_name
    p_user_id        -- Populate created_by
  );
